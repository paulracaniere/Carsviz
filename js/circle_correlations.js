function correlation_update(listOfIndices, correlations) {
    ...
    for (let i = 0; i < engineSpecs.length; i++) {
        index = listOfIndices.indexOf(i);
        if (index != -1) {
            let corPC1 = 0;
            let corPC2 = 0;
            let data_revelant = dataset.length;
            let name = engineSpecs[listOfIndices[index]];
            ...
            correlations[listOfIndices[index]].x = corPC1;
            correlations[listOfIndices[index]].y = corPC2;
            correlations[listOfIndices[index]].selected = true;
        } else {
...
function sigma_PC() {
    ...
    for (let id_data = 0; id_data < dataset.length; id_data++) {
        if (dataset[id_data].PC1 != 0 && dataset[id_data].PC2 != 0) {
